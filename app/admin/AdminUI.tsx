...
function loadStore(): Store {
  ...
  try { return JSON.parse(raw) as Store; } catch { return { metalPrices: [], pricingStructures: [], users: [] }; }
}
...
export default function AdminUI() {
  ...
  // Prices
  const addMetal = () => setStore(s => ({ ...s, metalPrices: [...s.metalPrices, { id: crypto.randomUUID(), name: "New Metal", unit: "oz", price: 0 }] }));
  const updateMetal = (id: string, data: Partial<MetalPrice>) =>
    setStore(s => ({ ...s, metalPrices: s.metalPrices.map(m => m.id === id ? { ...m, ...data } : m) }));
  const deleteMetal = (id: string) => setStore(s => ({ ...s, metalPrices: s.metalPrices.filter(m => m.id !== id) }));

  // Pricing Structures
  const addStructure = () => setStore(s => ({ ...s, pricingStructures: [...s.pricingStructures, { id: crypto.randomUUID(), name: "New Structure", multiplier: 1.0, description: "" }] }));
  const updateStructure = (id: string, data: Partial<PricingStructure>) =>
  ...
  // Users
  const addUser = () => setStore(s => ({ ...s, users: [...s.users, { id: crypto.randomUUID(), name: "New User", email: "", isAdmin: false }] }));
  const updateUser = (id: string, data: Partial<User>) =>
    setStore(s => ({ ...s, users: s.users.map(u => u.id === id ? { ...u, ...data } : u) }));
  const deleteUser = (id: string) => setStore(s => ({ ...s, users: s.users.filter(u => u.id !== id) }));
              ...
              <tbody>
                {store.metalPrices.map(m => (
                  <tr key={m.id} className="border-t">
                    <td className="py-2 pr-4"><Input value={m.name} onChange={e => updateMetal(m.id, { name: e.target.value })} /></td>
                    ...
                    <td className="py-2 pr-4"><Input type="number" step="0.01" value={m.price} onChange={e => updateMetal(m.id, { price: parseFloat(e.target.value) })} /></td>
                    <td className="py-2 pr-4"><OutlineButton onClick={() => deleteMetal(m.id)}>Delete</OutlineButton></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="space-y-4">
            {store.pricingStructures.map(p => (
              <div key={p.id} className="grid grid-cols-1 md:grid-cols-4 gap-3 border rounded-xl p-3 bg-white/60">
                ...
                <div className="flex items-center justify-end"><OutlineButton onClick={() => deleteStructure(p.id)}>Delete</OutlineButton></div>
              </div>
            ))}
          </div>
        </SectionCard>
                  ...
                  <tr key={u.id} className="border-t">
                    ...
                    <td className="py-2 pr-4">
                      <Select value={u.pricingStructureId ?? ""} onChange={e => updateUser(u.id, { pricingStructureId: e.target.value || undefined })}>
                        <option value="">—</option>
                        {store.pricingStructures.map(p => (<option key={p.id} value={p.id}>{p.name}</option>))}
                      </Select>
                    </td>
                    <td className="py-2 pr-4">
                      <label className="inline-flex items-center gap-2">
                        <input type="checkbox" checked={u.isAdmin} onChange={e => updateUser(u.id, { isAdmin: e.target.checked })} />
                        <span>Admin</span>
                      </label>
                    </td>
                    <td className="py-2 pr-4"><OutlineButton onClick={() => deleteUser(u.id)}>Delete</OutlineButton></td>
                  </tr>
                ))}
              </tbody>
            </table>
